...
import type { LVPData, Exercise, SetData } from '../types/index';
...
class SessionStartService {
  ...
  async getSessionStartData(
    ...
    return {
      ...
      bestVelocityAtLoad: bestVelocity ? {
        ...
        date: bestVelocity.date,
        reps: bestVelocity.reps,
      } : null,
      currentE1RM,
      mvt,
      dailyReadiness,
      historicalVelocityAtLoad: historicalVelocity.slice(0, 5), // Last 5 sessions
    };
  }
  ...
  private async calculateDailyReadiness(
    ...
    const similarLoadSets = recentSets.filter(
      (set: SetData) => Math.abs(set.load_kg - currentLoad) <= 5
    );
    ...
    const avgVelocityRecent3 = recent3.reduce(
      (sum: number, set: SetData) => sum + (set.avg_velocity || 0),
      0
    ) / recent3.length;
  ...
  private async getHistoricalVelocityAtLoad(
    lift: string,
    loadKg: number
  ): Promise<{ load: number; velocity: number; date: string }[]> {
    const recentSets = await this.db.getRecentSetsForLift(lift, 20);

    return recentSets
      .filter((set: SetData) => Math.abs(set.load_kg - loadKg) <= 1 && set.avg_velocity !== null)
      .map((set: SetData) => ({
        load: set.load_kg,
  ...
  async getReadinessTrend(lift: string): Promise<ReadinessTrendData> {
    ...
    const sessions = recentSets.map((set: SetData) => ({
      date: set.timestamp,
    ...
    const avgFirst = firstHalf.reduce((sum: number, s: { avg_velocity: number }) => sum + s.avg_velocity, 0) / firstHalf.length;
    const avgSecond = secondHalf.reduce((sum: number, s: { avg_velocity: number }) => sum + s.avg_velocity, 0) / secondHalf.length;
