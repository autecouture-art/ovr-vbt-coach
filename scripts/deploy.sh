...
EXPECTED_XCODE_APP="${REPVELO_XCODE_APP:-/Applications/Xcode.app}"
EXPECTED_DEVELOPER_DIR="$EXPECTED_XCODE_APP/Contents/Developer"
FASTLANE_KEY_PATH="$PROJECT_ROOT/fastlane/api_key.p8"
OUTPUT_IPA="$PROJECT_ROOT/ios/fastlane_export/RepVeloCoach.ipa"
FASTLANE_XCODEBUILD_SETTINGS_TIMEOUT="${FASTLANE_XCODEBUILD_SETTINGS_TIMEOUT:-20}"
FASTLANE_XCODEBUILD_SETTINGS_RETRIES="${FASTLANE_XCODEBUILD_SETTINGS_RETRIES:-6}"
...
main() {
  log "Starting TestFlight upload via fastlane beta"
  ...
  export FASTLANE_XCODEBUILD_SETTINGS_RETRIES

  bundle exec fastlane beta

  log "Lane beta finished successfully"
  if [[ -f "$OUTPUT_IPA" ]]; then
    log "Generated IPA: $OUTPUT_IPA"
  else
    log "IPA path not found after upload: $OUTPUT_IPA"
  fi
