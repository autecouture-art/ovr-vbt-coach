...
| 2026-06-03-01 | crash/vbt | VBT接続後にセッションモードへ入るとクラッシュする | implemented | critical | codex | src/hooks/useSessionLogic.ts, src/services/BLEService.ts | local_checked | 実機でVBT接続→セッション画面遷移を再確認待ち | 接続直後の欠損/NaN/異常VBTパケットがLive Data表示や音声読み上げのtoFixedへ到達しないよう、BLEパース側とSession入口で数値正規化・範囲ガードを追加 |
