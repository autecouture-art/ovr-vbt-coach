...
| 2026-04-13-15 | session | 自動スタートROM閾値を固定5cmから設定化 | implemented | high | claude | src/types/index.ts, src/services/AppSettingsService.ts, src/hooks/useSessionLogic.ts, app/(tabs)/settings.tsx, src/services/ExerciseService.ts, src/constants/exerciseCatalog.ts | untested | まだ評価待ち | AppSettings に全体デフォルト(5cm)、Exercise に種目別override、settingsタブで全体編集、種目マスターで個別編集、useSessionLogicで exercise.override 優先 |
